...
Parse.Cloud.afterSave('Activity', function(request) {
  ...
  toUser.fetch().then(function(fetching) {
        ...
        if (results.length > 0) {
            for (var i = 0; i < results.length; i++) {
                ...
                thirdQuery.equalTo('user', newUser);
                    ...
                    console.log("User defective? " + request.object.get('fromUser').id);
                    ...
                    Parse.Push.send({
  					...
  					.then(function() {
   	   					console.log('Sent comment push successfully');
  					}, function(error) {
 ...
 if (request.object.get("type") === "mention") {
    ...
    queryPush.containedIn('user', newUser);
    Parse.Push.send({
    ...
    console.log('Other query to ' + toUser.get('displayName'));

    queryPush.equalTo('user', toUser);
    Parse.Push.send({
        where: queryPush, // Set our Installation query.
        data: alertPayload(request)
    }, { useMasterKey: true })
